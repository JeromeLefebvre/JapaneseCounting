...
'''
oneToTenKanji = {10:u'じゅう',1:u'いち',2:u'に',3:u'さん',4:u'よん',5:u'ご',6:u'ろく',7:u'なな',8:u'はち',9:u'きゅう'}
oneToTenReading = {10:u'十',1:u'一',2:u'二',3:u'三',4:u'四',5:u'五',6:u'六',7:u'七',8:u'八',9:u'九'}
dakuten = {'た': 'だ', 'そ': 'ぞ', 'せ': 'ぜ', 'ヘ': 'ベ', 'す': 'ず', 'し': 'じ', 'さ': 'ざ', 'ヒ': 'ビ', 'け': 'げ', 'く': 'ぐ', 'ホ': 'ぼ', 'き': 'ぎ', 'か': 'が', 'ト': 'ド', 'テ': 'デ', 'ツ': 'ヅ', 'セ': 'ゼ', 'タ': 'ダ', 'ソ': 'ゾ', 'ほ': 'ぼ', 'へ': 'べ', 'ク': 'グ', 'シ': 'ジ', 'ハ': 'バ', 'ふ': 'ぶ', 'ひ': 'び', 'ケ': 'ゲ', 'は': 'ば', 'キ': 'ギ', 'カ': 'ガ', 'と': 'ど', 'て': 'で', 'つ': 'づ', 'チ': 'ヂ', 'ス': 'ズ', 'サ': 'ザ', 'フ': 'ブ', 'ち': 'ぢ'}
handakuten = {'は': 'ぱ', 'フ': 'プ', 'ヘ': 'ペ', 'ほ': 'ぽ', 'へ': 'ぺ', 'ハ': 'パ', 'ヒ': 'ピ', 'ふ': 'ぷ', 'ひ': 'ぴ', 'ホ': 'ポ'}
reverseHandakuten = {'ぽ': 'ほ', 'ポ': 'ホ', 'ぴ': 'ひ', 'パ': 'ハ', 'ぺ': 'へ', 'ピ': 'ヒ', 'プ': 'フ', 'ペ': 'ヘ', 'ぱ': 'は', 'ぷ': 'ふ'}
reverseDakuten = {'ガ': 'カ', 'ギ': 'キ', 'ど': 'と', 'づ': 'つ', 'で': 'て', 'だ': 'た','ぽ':'ほ','ポ':'ポ', 'ぢ': 'ち', 'ゼ': 'セ', 'ゾ': 'ソ', 'べ': 'へ', 'ジ': 'シ', 'ズ': 'ス', 'ぶ': 'ふ', 'げ': 'け', 'ば': 'は', 'び': 'ひ', 'ゲ': 'ケ', 'が': 'か', 'ぎ': 'き', 'ド': 'ト', 'ヂ': 'チ', 'ヅ': 'ツ', 'デ': 'テ', 'ダ': 'タ', 'グ': 'ク', 'ぜ': 'せ', 'バ': 'ハ', 'ぞ': 'そ', 'ベ': 'ヘ', 'じ': 'し', 'ざ': 'さ', 'ず': 'す', 'ブ': 'フ', 'ぐ': 'く', 'ビ': 'ヒ', 'ザ': 'サ'}
...
def count(theNumber, ichi = 2, debug=False):
	...
	# Not sure how to handle 1000**6, since there is there are two possible choices for the kanji, then for the larger number, the associated number is ambigious.
	listOfSteps = ((1000**5,'垓'),(10000**4,'京'),(10000**3,'兆'),(10000**2,'億'),(10000,'万'),(1000,'千'),(100,'百'),(10,'十'))
	reading = []
...
def removeDakuten(letter):
	reverseDakuten = {'ガ': 'カ', 'ギ': 'キ', 'ど': 'と', 'づ': 'つ', 'で': 'て', 'だ': 'た', 'ぢ': 'ち', 'ゼ': 'セ', 'ゾ': 'ソ', 'べ': 'へ', 'ジ': 'シ', 'ズ': 'ス', 'ぶ': 'ふ', 'げ': 'け', 'ば': 'は', 'び': 'ひ', 'ゲ': 'ケ', 'が': 'か', 'ぎ': 'き', 'ド': 'ト', 'ヂ': 'チ', 'ヅ': 'ツ', 'デ': 'テ', 'ダ': 'タ', 'グ': 'ク', 'ぜ': 'せ', 'バ': 'ハ', 'ぞ': 'そ', 'ベ': 'ヘ', 'じ': 'し', 'ざ': 'さ', 'ず': 'す', 'ブ': 'フ', 'ぐ': 'く', 'ビ': 'ヒ', 'ザ': 'サ'}
...
def dictionaryOfExceptions():
	...
	for line in exceptionsData.readlines():
		...
		if not line.startswith('#') and not line.startswith('＃') and line != '\n' and line:
			...
			# if there is multiple reading, it will be n a list and if there is not, it still needs to be in a list
...
def reading(integer,counterKanji,counterReading=''):
	...
	# If integer is greater than 10, and it is not an exception, then the reading is done simply as the number plus the counter
	if integer > 10:
		pass
	...
	def testFromFile(self):		
		...
		for line in fileData.readlines():
			...
			if not line.startswith('#') and line != '\n':
				...
				self.assertTrue(expected_reading in reading(int(counter_integer),counterKanji), "Failed here!!")
		fileData.close()
